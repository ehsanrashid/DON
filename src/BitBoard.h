...
#pragma region Attacks
    ...
    F_INLINE Bitboard attacks_sliding (Square s, const Delta deltas[], Bitboard occ = U64 (0))
    {
        ...
        {
            Square sq = s + del;
            while (_ok (sq) && square_dist(sq, sq - del) == 1)
            {
                attacks_slid += sq;
                if (occ & sq) break;
                sq += del;
    ...
    template<PType PT>
    // Attacks of the PAWN
    extern F_INLINE Bitboard attacks_bb (Color c, Square s);

    template<>
    // PAWN attacks
    F_INLINE Bitboard attacks_bb<PAWN> (Color c, Square s) { return _attacks_pawn_bb[c][s]; }

    // --------------------------------
    template<PType PT>
    // Attacks of the PType
    extern F_INLINE Bitboard attacks_bb (Square s);

    template<PType PT>
    // Attacks of the PType
    F_INLINE Bitboard attacks_bb (Square s) { return _attacks_type_bb[PT][s]; }
    // --------------------------------
    // explicit template instantiations
    template F_INLINE Bitboard attacks_bb<NIHT> (Square s);
    template F_INLINE Bitboard attacks_bb<BSHP> (Square s);
    template F_INLINE Bitboard attacks_bb<ROOK> (Square s);
    template F_INLINE Bitboard attacks_bb<QUEN> (Square s);
    template F_INLINE Bitboard attacks_bb<KING> (Square s);
    // --------------------------------

    template<PType PT>
    // Attacks of the PType with occupancy
    extern F_INLINE Bitboard attacks_bb (Square s, Bitboard occ);

    template<>
    // KNIGHT attacks
    F_INLINE Bitboard attacks_bb<NIHT> (Square s, Bitboard occ) { return _attacks_type_bb[NIHT][s]; }
    template<>
    // KING attacks
    F_INLINE Bitboard attacks_bb<KING> (Square s, Bitboard occ) { return _attacks_type_bb[KING][s]; }
    // --------------------------------
