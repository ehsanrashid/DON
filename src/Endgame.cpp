...
namespace {
    ...
    inline int pushToCorner(Square s) {
        return std::abs(7 - (sFile(s) + sRank(s)));
    }
...
template<> Value Endgame<KBNK>::operator()(Position const &pos) const {
    ...
    auto value{ VALUE_KNOWN_WIN
              + pushClose(skSq, wkSq)
              + 3520
              + 420 * pushToCorner(colorOpposed(sbSq, SQ_A1) ? flipFile(wkSq) : wkSq) };
...
template<> Value Endgame<KRKP>::operator()(Position const &pos) const {
    ...
    i32 sTempo{ pos.activeSide() == stngColor };
    i32 wTempo{ pos.activeSide() == weakColor };
...
template<> Value Endgame<KRKB>::operator()(Position const &pos) const {
    ...
    // && distance(wkSq, wbSq) == 1
    // && distance(skSq, wbSq) > 1) {
    //    return VALUE_DRAW;
    //}
...
template<> Value Endgame<KRKN>::operator()(Position const &pos) const {
    ...
    //if (distance(wkSq, wnSq) + sTempo <= 3
    // && distance(skSq, wnSq) > 1) {
    //    return VALUE_DRAW;
    //}
...
template<> Scale Endgame<KRPKR>::operator()(Position const &pos) const {
    ...
    if (spR <= RANK_5
     && wkSq == spSq + NORTH
     && distance(skSq, spSq) - sTempo >= 2
     && distance(skSq, wrSq) - sTempo >= 2) {
        return SCALE_DRAW;
    }
    ...
    if (spR <= RANK_4
     && wkSq > spSq) {
        ...
        if (distance<File>(wkSq, spSq) == 1
         && distance(skSq, wkSq) > 2) {
            return Scale(24
...
template<> Scale Endgame<KRPKB>::operator()(Position const &pos) const {
    ...
    if ((pos.pieces(stngColor, PAWN)
       & (FileBB[FILE_A]|FileBB[FILE_H])) != 0) {
        ...
        if (spR == RANK_6
         && distance(spSq + 2 * PawnPush[stngColor], wkSq) <= 1
         && contains(PieceAttackBB[BSHP][wbSq], spSq + PawnPush[stngColor])
         && distance<File>(wbSq, spSq) >= 2) {
            return Scale(8);
        }
    }
    ...
    if (colorOpposed(sbSq, wbSq)) {
        ...
        if (d == 1) {
            if (colorOpposed(wkSq, sbSq)) {
                ...
                if (wkSq == block1Sq
                 && (wbSq == block2Sq
                  || (pos.pieces(weakColor, BSHP)
                    & pos.pieceAttacksFrom(BSHP, block2Sq)) != 0
                ...
                if (wkSq == block2Sq
                 && (wbSq == block1Sq
                  || (pos.pieces(weakColor, BSHP)
                    & pos.pieceAttacksFrom(BSHP, block1Sq)) != 0)) {
                    return SCALE_DRAW;
                }
            }
        }
...
template<> Scale Endgame<KNPKB>::operator()(Position const &pos) const {
    ...
    if ((frontSquaresBB(stngColor, spSq)
       & pos.pieceAttacksFrom(BSHP, sbSq)) != 0) {
...
template<> Scale Endgame<KPsK>::operator()(Position const &pos) const {
    ...
    if (((sPawns & ~FileBB[FILE_A]) == 0
     ...
     && (sPawns & ~frontRanksBB(weakColor, wkSq)) == 0
     && distance<File>(wkSq, scanLSq(sPawns)) <= 1) {
        return SCALE_DRAW;
    }
...
template<> Scale Endgame<KQKRPs>::operator()(Position const &pos) const {
    ...
    if (relativeRank(weakColor, wkSq) <= RANK_2
     ...
     && (pos.pieces(weakColor, PAWN)
       ...
       & pos.pawnAttacksFrom(stngColor, wrSq)) != 0) {
        return SCALE_DRAW;
    }
