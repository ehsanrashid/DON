...
bool Position::pseudoLegal(Move m) const
{
    ...
    if (0 != checkers()) {
        ...
        return ENPASSANT != mType(m) ?
                ...
                contains(checkers() | betweenBB(scanLSq(checkers()), fkSq), dst) :
              ...
              || contains(betweenBB(scanLSq(checkers()), fkSq), dst));
    }
    return true;
}
...
void Position::setCastle(Color c, Square rookOrg) {
    ...
    cslKingPath[c][cs] = (betweenBB(kingOrg, kingDst)
                        | kingDst)
                       & ~kingOrg;
    cslRookPath[c][cs] = (betweenBB(kingOrg, kingDst)
                        | betweenBB(rookOrg, rookDst)
                        | kingDst
                        | rookDst)
                       & ~kingOrg
                       & ~rookOrg;
...
void Position::doMove(Move m, StateInfo &nsi, bool isCheck) {
    ...
    if (CASTLE == mType(m)) {
        ...
        auto captured = ENPASSANT != mType(m) ? PType[board[dst]] : PAWN;
        if (NONE != captured) {
            ...
            if (PAWN == captured) {
                if (ENPASSANT == mType(m)) {
                    ...
                    assert(PAWN == mpt
                        ...
                        && dst == epSquare()
                        && empty(dst) //&& !contains(pieces(), dst)
                        && (pasive|PAWN) == board[cap]); //&& contains(pieces(pasive, PAWN), cap));
                }
...
void Position::flip()
{
    ...
    iss >> token;
    if (token != "-") {
        token.replace(1, 1, { 1, toChar(flipRank(toRank(token[1]))) });
    }
    ff += token;
...
void Position::mirror()
{
    ...
    if (token != "-") {
        for (auto &ch : token) {
            if (Options["UCI_Chess960"]) {
                assert(isalpha(ch));
                ch = toChar(flipFile(toFile(char(tolower(ch)))), islower(ch));
            }
            else {
                switch (ch) {
                case 'K': ch = 'Q'; break;
    ...
    iss >> token;
    if (token != "-") {
        token.replace(0, 1, { 1, toChar(flipFile(toFile(token[0]))) });
    }
    ff += token;
    // 5-6. Halfmove clock and Fullmove number
    std::getline(iss, token, '\n');
