...
Value Worker::search(Position&    pos,
    ...
    {
        auto futility_margin = [&](bool ttMoveOK) noexcept {
            Value futilityMult = 70 + 21 * ttMoveOK;
        ...
        if (!ss->pvHit && depth < 14 && eval >= beta && !is_loss(alpha) && !is_win(eval)
            && eval - futility_margin(ttd.move != Move::None) >= beta)
...
int Worker::correction_value(const Position& pos, const Stack* const ss) noexcept {
    ...
    return std::clamp<std::int64_t>(
           + 5136LL * (   pawnCorrectionHistory[correction_index(pos.pawn_key(WHITE))][WHITE][ac]
