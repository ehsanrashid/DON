...
reduction(Depth depth, std::uint8_t moveCount, std::uint32_t deltaRatio, bool improve) noexcept {
    ...
    return 1182 + reductionScale - deltaRatio
         + int(!improve) * constexpr_lround(0.4648 * reductionScale);
}
...
void update_continuation_history(Stack* ss, Piece pc, Square dstSq, int bonus) noexcept {
    ...
    for (std::size_t i = 0; i < ContHistoryCount; ++i)
    {
        ...
        (*ssi->pieceSqHistory)[+pc][dstSq]
          << ContHistoryOffsets[i] + constexpr_lround(ContHistoryWeights[i] * bonus);
    }
}
...
Value adjust_eval_value(Value evalValue, int correctionValue) noexcept {
    return in_range(evalValue + constexpr_lround(7.6294e-6 * correctionValue));
}
           ...
           && (mainManager == nullptr || limit.depth == DEPTH_ZERO || rootDepth <= limit.depth))
    {
        ...
        for (curPV = 0; curPV < multiPV; ++curPV)
        {
            ...
            int delta = 5 + std::min(int(thread_count()) - 1, 8)  //
                      + constexpr_lround(1.1111e-4 * std ::abs(avgSqrValue));
...
Value Worker::search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, std::int8_t red, Move excludedMove) noexcept {
    ...
    if constexpr (!PVNode)
    {
            ...
            && is_ok(ttd.bound & fail_bound(ttd.value >= beta)))
        {
            ...
            if (pos.rule50_count() < constexpr_lround((1.0 - int(pos.has_rule50_high()) * 0.20)
                                                      * int(rule50_threshold())))
            {
    ...
    if (!(ss - 1)->inCheck && preOk && !preCapture)
    {
        ...
        if (!ttd.hit && preNonPawn)
            update_pawn_history(pawnHistory, pos[preSq], preSq, constexpr_lround(13.0000 * bonus));

        update_quiet_history(~ac, preMove, constexpr_lround(9.0000 * bonus));
    }
    ...
    if constexpr (!PVNode)
    {
            ...
            && (ttmNone || history_value(pos, ttd.move, ac, contHistory) >= 32768 - int(ttmCapture) * 25968))
        {
            ...
            int margin = depth * baseFutility                                                //
                       - constexpr_lround((int(improve) * 2.4160 + int(worsen) * 0.3232) * baseFutility)  //
                       + constexpr_lround(5.7252e-6 * absCorrectionValue);
            if (margin < 0)
                margin = 0;
    ...
    if constexpr (CutNode)
    {
            ...
            && !is_loss(beta) && ss->evalValue - 350 + 18 * depth >= beta)
        {
            ...
            Depth R = std::min(7 + constexpr_lround(0.33334 * int(depth)), int(depth));
    ...
    if (depth > 2 && !is_loss(beta))
    {
        ...
        {
        Depth probCutDepth = std::clamp(depth - 5 - constexpr_lround(3.1746e-3 * int(ss->evalValue - beta)), 0, int(depth));
    ...
    while ((move = mp.next_move()) != Move::None)
    {
        ...
        if constexpr (!RootNode)
        {
            if (hasNonPawn && !is_loss(bestValue))
            {
                ...
                if (capture)
                {
                    ...
                    if (!check && lmrDepth < 7)
                    {
                        int futility = 232 + ss->evalValue + piece_value(capturedPt)
                                     + 217 * lmrDepth + constexpr_lround(0.1279 * history);
                        if (futility <= alpha)
                            continue;
                    }

                    // SEE based pruning for captures and checks
                    int threshold = 166 * depth + constexpr_lround(34.4828e-3 * history);
                ...
                else
                {
                    ...
                    history += constexpr_lround(2.1563 * quietHistory[ac][move.raw()]);

                    // (*Scaler) Generally, higher history scales well
                    lmrDepth += constexpr_lround(3.1172e-4 * history);
        ...
        {
            Value singularAlpha = std::max(ttd.value - 1 - constexpr_lround((0.8833 + int(!PVNode && ss->ttPv) * 1.2500) * int(depth)), -VALUE_INFINITE);
            ...
            {
                int corrMargin = constexpr_lround(4.3351e-6 * absCorrectionValue);

                int doubleMargin = -4 + int(PVNode) * 199 - int(!ttmCapture) * 201 - corrMargin - int(ss->ply > rootDepth) * 42 - constexpr_lround(7.0271e-3 * ttMoveHistory);
        ...
        r += 714;
        r -= 73 * moveCount;
        r -= constexpr_lround(32.9272e-6 * absCorrectionValue);

        // (*Scaler) Decrease reduction if position is or has been on the PV
        r -= int(ss->ttPv)
           * (2719 + int(PVNode) * 983 + int(is_valid(ttd.value) && ttd.value > alpha) * 922
              + int(ttd.depth >= depth) * (934 + int(CutNode) * 1011));
        ...
        if (ss->cutoffCount > 1)
            r += (256 << std::min(ss->cutoffCount - 2, 8)) + int(AllNode) * 1024;

        // For first picked move (ttMove) reduce reduction
        r -= int(ttm) * 2151;

        // Decrease/Increase reduction for moves with a good/bad history
        r -= constexpr_lround(103.7598e-3 * ss->history);
    ...
    else if (preOk)
    {
        ...
        if (!preCapture)
        {
            ...
            int bonusScale = -215
                            ...
                            - constexpr_lround(0.01 * (ss - 1)->history);
            // clang-format on
            if (bonusScale < 0)
                bonusScale = 0;
            ...
            if (preNonPawn)
                update_pawn_history(pawnHistory, pos[preSq], preSq,
                                    constexpr_lround(35.4004e-3 * bonus));

            update_quiet_history(~ac, preMove, constexpr_lround(7.4158e-3 * bonus));

            update_continuation_history(ss - 1, pos[preSq], preSq,
                                        constexpr_lround(12.3901e-3 * bonus));
        }
...
void Worker::update_quiet_histories(const Position& pos, PawnHistory& pawnHistory, Stack* ss, Move m, int bonus) noexcept {
    ...
    update_pawn_history(pawnHistory, pos.moved_pc(m), m.dst_sq(), constexpr_lround((bonus > 0 ? 0.8837 : 0.4932) * bonus));

    update_quiet_history(pos.active_color(), m, constexpr_lround(1.0000 * bonus));

    update_low_ply_quiet_history(ss->ply, m, constexpr_lround(0.7861 * bonus));

    update_continuation_history(ss, pos.moved_pc(m), m.dst_sq(), constexpr_lround(0.8750 * bonus));
}
...
void Worker::update_histories(const Position& pos, PawnHistory& pawnHistory, Stack* ss, Depth depth, Move bestMove, const StdArray<SearchedMoves, 2>& searchedMoves) noexcept {
    ...
    {
        update_capture_history(pos, bestMove, constexpr_lround(1.3623 * bonus));
    }
    else
    {
        update_quiet_histories(pos, pawnHistory, ss, bestMove, constexpr_lround(0.8887 * bonus));

        // Decrease history for all non-best quiet moves
        int quietMalus = constexpr_lround(1.0596 * malus);
        for (Move qm : searchedMoves[0])
        {
            update_quiet_histories(pos, pawnHistory, ss, qm, -quietMalus);
            quietMalus = constexpr_lround(0.9326 * quietMalus);
        }
    }

    // Decrease history for all non-best capture moves
    for (Move cm : searchedMoves[1])
        update_capture_history(pos, cm, -constexpr_lround(1.4141 * malus));
    ...
    if (preOk && pos.captured_pc() == Piece::NO_PIECE && (ss - 1)->moveCount == 1 + int((ss - 1)->ttMove != Move::None))
        update_continuation_history(ss - 1, pos[preSq], preSq, -constexpr_lround(0.5879 * malus));
}
...
void Worker::update_correction_histories(const Position& pos, Stack* ss, int bonus) noexcept {
    ...
    histories.    pawn_correction<WHITE>(pos.    pawn_key(WHITE))[ac] << constexpr_lround(1.0000 * bonus);
    histories.    pawn_correction<BLACK>(pos.    pawn_key(BLACK))[ac] << constexpr_lround(1.0000 * bonus);
    histories.   minor_correction<WHITE>(pos.   minor_key(WHITE))[ac] << constexpr_lround(1.2188 * bonus);
    histories.   minor_correction<BLACK>(pos.   minor_key(BLACK))[ac] << constexpr_lround(1.2188 * bonus);
    histories.non_pawn_correction<WHITE>(pos.non_pawn_key(WHITE))[ac] << constexpr_lround(1.3906 * bonus);
    histories.non_pawn_correction<BLACK>(pos.non_pawn_key(BLACK))[ac] << constexpr_lround(1.3906 * bonus);
    ...
    h2[+prePc][preSq] << int(preOk) * constexpr_lround(0.9922 * bonus);
    h4[+prePc][preSq] << int(preOk) * constexpr_lround(0.4609 * bonus);
}
