...
namespace PawnStructure {
    ...
    {
        return (pawns) ? shift_del<DEL_S> (~occ) & pawns : bb_NULL;
    }
    template<> Bitboard pawns_pushable_sgl<BLACK> (Bitboard pawns, Bitboard occ)
    {
        return (pawns) ? shift_del<DEL_N> (~occ) & pawns : bb_NULL;
    }
    ...
    {
        return (pawns) ? (pawns & pawns_attacks<WHITE> (pawns)) : bb_NULL;
    }
    template<> Bitboard pawns_defended<BLACK> (Bitboard pawns)
    {
        return (pawns) ? (pawns & pawns_attacks<BLACK> (pawns)) : bb_NULL;
    }
    ...
    {
        return (pawns) ? (pawns_defended<WHITE> (pawns) & pawns_defending<WHITE> (pawns)) : bb_NULL;
    }
    template<> Bitboard pawns_defended_defending<BLACK> (Bitboard pawns)
    {
        return (pawns) ? (pawns_defended<BLACK> (pawns) & pawns_defending<BLACK> (pawns)) : bb_NULL;
    }
    ...
    {
        return (pawns) ? (pawns_defending<WHITE> (pawns) & ~pawns_defended<WHITE> (pawns)) : bb_NULL;
    }
    template<> Bitboard pawns_defending_not_defended<BLACK> (Bitboard pawns)
    {
        return (pawns) ? (pawns_defending<BLACK> (pawns) & ~pawns_defended<BLACK> (pawns)) : bb_NULL;
    }


    template<> Bitboard pawns_attacking<WHITE> (Bitboard pawns, Bitboard pieces)
    {
        return (pawns && pieces) ? (pawns & pawns_attacks<BLACK> (pieces)) : bb_NULL;
    }
    template<> Bitboard pawns_attacking<BLACK> (Bitboard pawns, Bitboard pieces)
    {
        return (pawns && pieces) ? (pawns & pawns_attacks<WHITE> (pieces)) : bb_NULL;
    }

    template<> Bitboard pawns_rammed<WHITE> (Bitboard wpawns, Bitboard bpawns)
    {
        return (wpawns && bpawns) ? (shift_del<DEL_S> (bpawns) & wpawns) : bb_NULL;
    }
    template<> Bitboard pawns_rammed<BLACK> (Bitboard wpawns, Bitboard bpawns)
    {
        return (wpawns && bpawns) ? (shift_del<DEL_N> (bpawns) & wpawns) : bb_NULL;
    }
