...
namespace Searcher
{
    ...
    void clear()
    {
        ...
        TT.clear();
        Threadpool.clear();
        SyzygyTB::initialize(string(Options["SyzygyPath"])); // Free up mapped files
    }
}
...
void Thread::search()
{
         ...
         || rootDepth <= Threadpool.limit.depth))
    {
        ...
        if (nullptr != mainThread)
        {
            ...
            if (mainThread->skillMgr.enabled()
             && mainThread->skillMgr.canPick(rootDepth))
            {
                mainThread->skillMgr.clearBestMove();
             ...
             && !mainThread->stopOnPonderhit)
            {
                ...
                if (elapsedTime > availableTime * (1 < rootMoves.size()))
                {
                ...
                if (elapsedTime > availableTime * 0.60)
                {
                    if (!mainThread->ponder)
                    {
    ...
    */
    ...
    else
    {
        ...
        if (think)
        {
            ...
            skillMgr.setLevel(bool(Options["UCI_LimitStrength"]) ?
