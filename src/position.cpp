...
std::string Position::fen(const bool complete) const noexcept {
    ...
    {
        const auto append_castling = [this, &fens](const Color        c,
                                                   const CastlingSide cs) noexcept -> void {
            if (has_castling_rights(c, cs))
            {
                const auto castlingCh =
                  Chess960 ? to_char(c, castling_rook_sq(c, cs)) : to_char(c, cs);
                fens.push_back(castlingCh);
            }
        };

        append_castling(WHITE, CastlingSide::KING);
        append_castling(WHITE, CastlingSide::QUEEN);
        append_castling(BLACK, CastlingSide::KING);
        append_castling(BLACK, CastlingSide::QUEEN);
    }
    else
        fens.push_back('-');
    ...
    if (is_ok(en_passant_sq()))
        fens.append(to_string(en_passant_sq()));
    else
        fens.push_back('-');
...
void Position::set_castling_rights(const Color c, const Square rookOrgSq) noexcept {
    ...
    const auto cs = make_cs(kingOrgSq, rookOrgSq);
    assert(!is_ok(castling_rook_sq(c, cs)));

    const auto cr = make_cr(c, cs);
    ...
    const Bitboard kingPathBB = Attacks::between_bb(kingOrgSq, kingDstSq);
    const Bitboard rookPathBB = Attacks::between_bb(rookOrgSq, rookDstSq);
...
void Position::set_state() noexcept {
    ...
    for (const Color c : {WHITE, BLACK})
    {
        for (const auto pt : PIECE_TYPES)
        {
            ...
            while (bb != 0)
            {
                ...
                const Key key = Zobrist_.piece_square(c, pt, s);
                assert(key != 0);
    ...
    st->key ^= Zobrist_.castling(castling_rights());

    st->key ^= Zobrist_.enpassant(en_passant_sq());
...
Dirties Position::do_move(const Move          m,
    ...
    Piece        capturedPc = piece(mt != Move::Type::EN_PASSANT ? dstSq : dstSq - pawn_spush(ac));
    ...
    if (mt == Move::Type::CASTLING)
    {
        ...
        movedKey          = Zobrist_.piece_square(ac, movedPt, orgSq)  //
                          ^ Zobrist_.piece_square(ac, movedPt, dstSq);
        const Key rookKey = Zobrist_.piece_square(ac, ROOK, rookOrgSq)  //
                          ^ Zobrist_.piece_square(ac, ROOK, rookDstSq);
    ...
    if (capture)
    {
        ...
        Key capturedKey = Zobrist_.piece_square(~ac, capturedPt, capturedSq);

        // If the captured piece is a pawn, update pawn hash key,
        // otherwise update non-pawn material.
        if (capturedPt == PAWN)
        {
            if (mt == Move::Type::EN_PASSANT)
            {
                ...
                capturedKey = Zobrist_.piece_square(~ac, capturedPt, capturedSq);
            }
        ...
        st->materialKeys[~ac] ^= Zobrist_.piece_count(~ac, capturedPt, count(~ac, capturedPt) - 1);
    ...
    if (movedPt == PAWN)
    {
        if (mt == Move::Type::PROMOTION)
        {
            ...
            st->materialKeys[ac] ^= Zobrist_.piece_count(ac, PAWN, count(ac, PAWN) - 1)
                                  ^ Zobrist_.piece_count(ac, promotedPt, count(ac, promotedPt));
        }
    ...
    if (is_ok(enPassantSq))
    {
        ...
        {
            st->enPassantSq = enPassantSq;
            st->key ^= Zobrist_.enpassant(enPassantSq);
        }
...
void Position::undo_move(const Move m) noexcept {
    ...
    Square       dstSq = m.dst_sq();
    const auto   mt    = m.type();
    assert(empty(orgSq) || mt == Move::Type::CASTLING);
    ...
    return (checkersBB == 0
            ...
            || (!more_than_one(checkersBB)
                ...
                && ((Attacks::between_bb(kingSq, lsq(checkersBB)) & dstSq) != 0
                    || (mt == Move::Type::EN_PASSANT
                        && (checkersBB & (dstSq - pawn_spush(ac))) != 0))))
...
bool Position::check(const Move m) const noexcept {
    ...
    if (
      ...
      (checks_bb(mt != Move::Type::PROMOTION ? type_of(piece(orgSq)) : m.promotion_type()) & dstSq)
        != 0
      // Is there a discovered check?
      || ((blockers_bb(~ac) & orgSq) != 0
          && (!Attacks::aligned(kingSq, orgSq, dstSq) || mt == Move::Type::CASTLING)))
        return true;
...
Key Position::move_key(const Move m) const noexcept {
    Key moveKey = st->key ^ Zobrist_.turn() ^ Zobrist_.enpassant(en_passant_sq());
    ...
    const auto   mt         = m.type();
    const Piece  movedPc    = piece(orgSq);
    const Square capturedSq = mt != Move::Type::EN_PASSANT ? dstSq : dstSq - pawn_spush(ac);
    ...
    moveKey ^=
      Zobrist_.piece_square(ac, movedPt, orgSq)
      ^ Zobrist_.piece_square(ac, mt != Move::Type::PROMOTION ? movedPt : m.promotion_type(),
                              mt != Move::Type::CASTLING ? dstSq : king_castle_sq(orgSq, dstSq))
      ^ Zobrist_.castling(castling_rights())
      ^ Zobrist_.castling(castling_rights() & ~castling_rights_mask(orgSq, dstSq));
    ...
    if (mt == Move::Type::CASTLING)
    {
        ...
        return moveKey  //
             ^ Zobrist_.piece_square(ac, ROOK, dstSq)
             ^ Zobrist_.piece_square(ac, ROOK, rook_castle_sq(orgSq, dstSq))
             ^ Zobrist_.mr50(rule50_count() + 1);
    }
        ...
        && enpassant_possible<false>(~ac, dstSq - pawn_spush(ac)))
    {
        ...
        return moveKey ^ Zobrist_.enpassant(dstSq);
    }

    return moveKey  //
         ^ Zobrist_.piece_square(~ac, type_of(capturedPc), capturedSq)
         ^ Zobrist_.mr50(movedPt == PAWN || is_ok(capturedPc) ? 0 : rule50_count() + 1);
}
...
bool Position::see_ge(const Move m, const int threshold) const noexcept {
    ...
    swap = piece_value(mt != Move::Type::PROMOTION ? movedPt : m.promotion_type()) - swap;
    ...
    occupancyBB ^= make_bb(orgSq, dstSq);
    ...
    const auto [bAttacksBB, rAttacksBB] = Attacks::attacks_bb_pair(dstSq);
    ...
    while (true)
    {
                ...
                || !Attacks::aligned(square<KING>(ac), orgSq, dstSq)))
        {
                    ...
                    || (Attacks::attacks_bb(dstSq, pt, occupancyBB) & square<KING>(ac)) == 0))
            {
                ...
                const auto [bNewAttacksBB, rNewAttacksBB] = Attacks::attacks_bb_pair(dstSq);
        ...
        else if ((b = pieces_bb(QUEEN) & acAttackersBB) != 0)
        {
            ...
            const auto [bNewAttacksBB, rNewAttacksBB] =
              Attacks::attacks_bb_pair(dstSq, occupancyBB);
...
bool Position::is_upcoming_repetition(i16 ply) const noexcept {
    ...
    for (u16 i = 3; i <= end; i += 2)
    {
        ...
        const usize index = Cuckoos_.find_key(moveKey);

        if (index >= Cuckoos_.size())
            continue;

        Move m = Cuckoos_[index].move;
        assert(m != Move::None);
...
Key Position::compute_material_key(const Color c) const noexcept {
    ...
    for (const auto pt : EX_KING_PIECE_TYPES)
        for (u8 cnt = 0; cnt < count(c, pt); ++cnt)
            materialKey ^= Zobrist_.piece_count(c, pt, cnt);
...
Key Position::compute_key() const noexcept {
    ...
    while (bb != 0)
    {
        ...
        key ^= Zobrist_.piece_square(pc, s);
    }

    key ^= Zobrist_.castling(castling_rights());

    key ^= Zobrist_.enpassant(en_passant_sq());
...
Key Position::compute_pawn_key() const noexcept {
    ...
    while (bb != 0)
    {
        ...
        pawnKey ^= Zobrist_.piece_square(pc, s);
    }
...
Key Position::compute_minor_key() const noexcept {
    ...
    while (bb != 0)
    {
        ...
        minorKey ^= Zobrist_.piece_square(pc, s);
    }
...
Key Position::compute_major_key() const noexcept {
    ...
    while (bb != 0)
    {
        ...
        majorKey ^= Zobrist_.piece_square(pc, s);
    }
...
Key Position::compute_non_pawn_key() const noexcept {
    ...
    while (bb != 0)
    {
        ...
        nonPawnKey ^= Zobrist_.piece_square(pc, s);
    }
...
std::ostream& operator<<(std::ostream& os, const Position& pos) noexcept {
    ...
    if (Bitboard checkersBB = pos.checkers_bb(); checkersBB != 0)
        while (checkersBB != 0)
            os << to_string(pop_lsq(checkersBB)) << " ";
    else
        os << "-";
...
void Position::dump(std::ostream& os) const noexcept {
    ...
    for (const Color c : {WHITE, BLACK})
    {
        for (const CastlingSide cs : {CastlingSide::KING, CastlingSide::QUEEN})
        {
            ...
            os << '\n';
            os << (is_ok(castlings.rookSq[c][+cs]) ? to_string(castlings.rookSq[c][+cs]) : "-");
            os << '\n';
        }
        os << '\n';
    }
