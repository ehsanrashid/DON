...
class Worker final {
   ...
   private:
    ...
    void update_pawn_history(std::uint32_t pawnIndex, Piece pc, Square dstSq, int bonus) noexcept;
    void update_low_ply_quiet_history(std::int16_t ssPly, Move m, int bonus) noexcept;

    void update_quiet_histories(const Position& pos, Stack* const ss, std::uint32_t pawnIndex, Move m, int bonus) noexcept;
    void update_histories(const Position& pos, Stack* const ss, std::uint32_t pawnIndex, Depth depth, Move bestMove, const StdArray<SearchedMoves, 2>& searchedMoves) noexcept;
