...
const std::vector<std::string> Positions{
  ...
  "4k3/3q1r2/1N2r1b1/3ppN2/2nPP3/1B1R2n1/2R1Q3/3K4 w - - 5 4",
...
// - TT size in MB,
// - number of search threads that should be used,
// - the limit value spent for each position,
// - a file name where to look for positions in FEN format,
...
Commands setup_bench(std::istringstream& iss, std::string_view currentFen) noexcept {
    ...
    std::string filename  = (iss >> token) ? lower_case(token) : "default";
    std::string limitType = (iss >> token) ? lower_case(token) : "depth";

    std::string goCommand = limitType == "eval" ? "eval" : "go " + limitType + " " + limitVal;
    ...
    if (limitType != "eval")
    {
        ...
        commands.emplace_back("setoption name Hash value " + hash);
    ...
    for (const auto& fen : fens)
    {
        ...
        else
        {
            ...
            commands.emplace_back(goCommand);
        }
    }
...
Setup setup_benchmark(std::istringstream& iss) noexcept {
    ...
    constexpr std::size_t DESIRED_TIME       = 150;
    ...
    if (!(iss >> desiredTime))
        ...
        setup.originalInvocation += ' ' + std::to_string(desiredTime);

    setup.filledInvocation += std::to_string(setup.threads) + ' '  //
                            + std::to_string(setup.ttSize) + ' '   //
                            + std::to_string(desiredTime);

    auto getCorrectedTime = [](std::uint16_t ply) -> double {
    ...
    for (const auto& game : Games)
    {
        ...
        {
            double correctedTime = getCorrectedTime(ply);
    ...
    double timeScaleFactor = (desiredTime * 1000.0) / totalTime;
    for (const auto& game : Games)
    {
        ...
        for (const auto& fen : game)
        {
            ...
            size_t correctedTime = getCorrectedTime(ply) * timeScaleFactor;
