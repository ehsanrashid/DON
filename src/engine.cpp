...
Engine::Engine(std::optional<std::string> path) noexcept :
    ...
    options.add("NumaPolicy",           Option("auto", "var none var auto var system var hardware var default", Option::OnChange([this](const Option& o) {
        set_numa_config(o);
        return get_numa_config_info_str() + '\n'
             + get_thread_allocation_info_str();
    })));
    options.add("Threads",              Option(MIN_THREADS, MIN_THREADS, MAX_THREADS, Option::OnChange([this](const Option&) {
        resize_threads_tt();
        return get_thread_allocation_info_str();
    })));
    options.add("Hash",                 Option(16, MIN_HASH, MAX_HASH, Option::OnChange([this](const Option& o) {
        resize_tt(o);
        return "Hash: " + std::to_string(int(o));
    })));
    options.add("Clear Hash",           Option(Option::OnChange([this](const Option&) { init(); return std::nullopt; })));
    options.add("HashRetain",           Option(false));
    options.add("HashFile",             Option(""));
    options.add("Save Hash",            Option(Option::OnChange([this](const Option&) { return save_hash() ? "Save succeeded" : "Save failed"; })));
    options.add("Load Hash",            Option(Option::OnChange([this](const Option&) { return load_hash() ? "Load succeeded" : "Load failed"; })));
    ...
    options.add("DrawMoveCount",        Option(Position::DrawMoveCount, 5, 50, Option::OnChange([](const Option& o) { Position::DrawMoveCount = int(o); return std::nullopt; })));
    options.add("UCI_Chess960",         Option(Position::Chess960,             Option::OnChange([](const Option& o) { Position::Chess960 = bool(o); return std::nullopt; })));
    options.add("UCI_LimitStrength",    Option(false));
    options.add("UCI_ELO",              Option(Skill::MaxELO, Skill::MinELO, Skill::MaxELO));
    options.add("UCI_ShowWDL",          Option(false));
    options.add("OwnBook",              Option(false));
    options.add("BookFile",             Option("", Option::OnChange([](const Option& o) { return Book.load(o) ? "Load succeeded" : "Load failed"; })));
    options.add("BookProbeDepth",       Option(100, 1, 256));
    options.add("BookPickBest",         Option(true));
    options.add("SyzygyPath",           Option("", Option::OnChange([](const Option& o) { Tablebases::init(o); return std::nullopt; })));
    options.add("SyzygyProbeLimit",     Option(7, 0, 7));
    options.add("SyzygyProbeDepth",     Option(1, 1, 100));
    options.add("Syzygy50MoveRule",     Option(true));
    options.add("SyzygyPVExtend",       Option(true));
    options.add("BigEvalFile",          Option(EvalFileDefaultNameBig  , Option::OnChange([this](const Option& o) { load_big_network(o);   return std::nullopt; })));
    options.add("SmallEvalFile",        Option(EvalFileDefaultNameSmall, Option::OnChange([this](const Option& o) { load_small_network(o); return std::nullopt; })));
    options.add("ReportMinimal",        Option(false));
    options.add("LogFile",              Option("", Option::OnChange([](const Option& o) { Logger::start(o); return std::nullopt; })));
    options.add("Stop Logger",          Option(    Option::OnChange([](const Option&)   { Logger::stop(); return std::nullopt; })));
    // clang-format on
