...
Engine::Engine(const fs::path& path) noexcept :
    ...
    options().add("NumaPolicy",        OptionFactory::string("auto", Option::OnChange([this](const Option& o) { return set_numa_config(o) ? numa_config_info() + '\n' + thread_allocation() : "NumaPolicy: invalid value '" + std::string(o) + "', keeping previous config."; })));
    options().add("Threads",           OptionFactory::spin(1, 1, int(THREAD_MAX), Option::OnChange([this](const Option&) { resize_threads_tt(); return thread_allocation(); })));
    options().add("Hash",              OptionFactory::spin(16, 1, int(HASH_MAX), Option::OnChange([this](const Option& o) { resize_tt(o); return "Hash: " + std::to_string(int(o)); })));
    options().add("Clear Hash",        OptionFactory::button(Option::OnChange([this](const Option&) { reset(); return std::nullopt; })));
    options().add("HashRetain",        OptionFactory::check(false));
    options().add("HashFile",          OptionFactory::string(""));
    options().add("Save Hash",         OptionFactory::button(Option::OnChange([this](const Option&) { return save_hash(utf8_to_path(options()["HashFile"])) ? "Save succeeded" : "Save failed"; })));
    options().add("Load Hash",         OptionFactory::button(Option::OnChange([this](const Option&) { return load_hash(utf8_to_path(options()["HashFile"])) ? "Load succeeded" : "Load failed"; })));
    options().add("Ponder",            OptionFactory::check(false));
    options().add("MultiPV",           OptionFactory::spin(1, 1, int(MOVE_MAX)));
    options().add("UCI_Chess960",      OptionFactory::check(Position::Chess960, Option::OnChange([](const Option& o) { Position::Chess960 = bool(o); return std::nullopt; })));
    ...
    options().add("HistoryLoadFactor", OptionFactory::spin(75, 10, 100, Option::OnChange([this](const Option&) { set_history_max_load_factor(); return std::nullopt; })));
    options().add("DrawMoveCount",     OptionFactory::spin(Position::DrawMoveCount, 5, 50, Option::OnChange([](const Option& o) { Position::DrawMoveCount = int(o); return std::nullopt; })));
    options().add("Book",              OptionFactory::check(false));
    options().add("BookFile",          OptionFactory::string("", Option::OnChange([](const Option& o) { return load_book(utf8_to_path(o)) ? "Load succeeded" : "Load failed"; })));
    options().add("BookProbeDepth",    OptionFactory::spin(100, 1, 256));
    options().add("BookBestPick",      OptionFactory::check(true));
    options().add("SyzygyPath",        OptionFactory::string("", Option::OnChange([](const Option& o) { Tablebase::Syzygy::init(o); return std::nullopt; })));
    ...
    options().add("EvalFile",          OptionFactory::string(EvalFileDefaultName, Option::OnChange([this](const Option& o) { load_network(utf8_to_path(o)); return std::nullopt; })));
    options().add("MinimalInfo",       OptionFactory::check(false));
    options().add("LogFile",           OptionFactory::string("", Option::OnChange([](const Option& o) { return Logger::start(utf8_to_path(o)) ? "Logger started" : "Logger not started"; })));
    options().add("Stop Logger",       OptionFactory::button(Option::OnChange([](const Option&) { Logger::stop(); return std::nullopt; })));
    // clang-format on
