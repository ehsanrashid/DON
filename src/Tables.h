...
{
private:
    using NestedArray = typename Array<T, Sizes...>::type;

public:
    using type = std::array<NestedArray, Size>;
...
using Table = typename Array<T, Sizes...>::type;
...
using PieceSquareTypeStatsTable = StatsTable<i16, 10692, MAX_PIECE, SQUARES, PIECE_TYPES>;

/// PieceSquareStatsTable store moves history according to piece.
/// indexed by [piece][square]
using PieceSquareStatsTable = StatsTable<i16, 29952, MAX_PIECE, SQUARES>;
...
using ContinuationStatsTable = Table<PieceSquareStatsTable, MAX_PIECE, SQUARES>;

/// PieceSquareMoveTable stores moves, indexed by [piece][square]
using PieceSquareMoveTable = Table<Move, MAX_PIECE, SQUARES>;
