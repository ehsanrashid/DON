...
alignas(CACHE_LINE_SIZE) Array<MagicMask, TABLE_SIZES[0] + TABLE_SIZES[1]> ATTACK_TABLE;

alignas(CACHE_LINE_SIZE) Array<TableView<MagicMask>, 2> TABLE_VIEWS{
  TableView{ATTACK_TABLE.data() + 0x000000000000, TABLE_SIZES[0]},
  TableView{ATTACK_TABLE.data() + TABLE_SIZES[0], TABLE_SIZES[1]}  //
};
...
void init_magics() noexcept {
    ...
    for (Square s = SQ_A1; s <= SQ_H8; ++s)
    {
        ...
        magic.attacksBBs = s == SQ_A1 ? TABLE_VIEWS[PT - BISHOP].data()  //
