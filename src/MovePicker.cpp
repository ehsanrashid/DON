...
MovePicker::MovePicker(const Position &p
                     , Move ttm
                     , Depth d
                     , const Table<const PieceSquareStatsTable*, 6> &ps
                     , const Table<Move, 2> &km
                     , Move cm)
    : pos(p)
    , ttMove(ttm)
    , depth(d)
    ...
    for (auto &vm : vmoves)
    {
        if (GenType::CAPTURE == GT)
        {
            ...
            vm.value = 6 * i32(PieceValues[MG][pos.captureType(vm)])
                        + thread->captureStats[pos[orgSq(vm)]][dstSq(vm)][pos.captureType(vm)];
        }
        else
        if (GenType::QUIET == GT)
        {
            vm.value = thread->quietStats[pos.active][mIndex(vm)]
        ...
        else // GenType::EVASION == GT
        {
            vm.value = pos.capture(vm) ?
                      ...
                      - pType(pos[orgSq(vm)]) :
                        thread->quietStats[pos.active][mIndex(vm)]
...
Move MovePicker::nextMove()
{
    ...
    case QUIESCENCE_INIT:
        ...
        vmoves.erase(
            std::remove_if(vmoves.begin(), vmoves.end(),
                            [&](const ValMove &vm)
                            { return ttMove == vm
                                    || !pos.fullLegal(vm); }), vmoves.end());
    ...
    case NATURAL_GOOD_CAPTURES:
        ...
        refutationMoves.erase(
            std::remove_if(refutationMoves.begin(), refutationMoves.end(),
                            [&](Move m)
                            { return MOVE_NONE == m
                                || ttMove == m
                                || pos.capture(m)
                                || !pos.pseudoLegal(m)
                                || !pos.legal(m); }), refutationMoves.end());
